estimateur_3aprime <- function(donnees, scenarios, variantes) {
  ...
  for (i in scenarios) {
    ...
    donnees[[paste0("poids_3aprime_cnr_exacte_", i)]] <- 1 / (proba_multi_cnr_exacte  + proba_mono_cnr_exacte)
    
    donnees[[paste0("poids_3aprime_sans_grh_", i)]] <- 1 / (proba_multi_sans_grh + proba_mono_sans_grh)
