boucles_simulations <- function(nb_sim,
  ...
  if (parallel) {
    ...
    if (is.null(batch_size)) {
      batch_size <- max(2, ceiling(nb_sim / n_cores))
    }
    ...
    cat(sprintf(
      "Simulations: %d en %d batches (taille: ~%d)\n",
    ...
    light_vars <- c(
      ...
      "source_files", "temp_files", "batches"
    )
    ...
    parallel::clusterExport(cl, c("process_batch", "process_batch_with_progress"), envir = environment())

    cat("Lancement des simulations...\n")
    results_batched <- vector("list", length(batches))

    for (i in seq_along(batches)) {
      batch_result <- parallel::clusterApplyLB(cl, list(batches[[i]]), process_batch_with_progress)[[1]]
      results_batched[[i]] <- batch_result
      cat(sprintf("\u2713 Batch %d OK\n", i))
    }
